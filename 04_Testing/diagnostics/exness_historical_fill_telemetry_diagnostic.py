"""
===============================================================================
Module      : exness_historical_fill_telemetry_diagnostic.py
...
Diagnostic direct-script support
--------------------------------
The shared marker-based diagnostic bootstrap discovers the repository root and
adds it to sys.path before production imports, so both invocation styles work
from the repository root:

    python -m 04_Testing.diagnostics.exness_historical_fill_telemetry_diagnostic

and:

    python 04_Testing/diagnostics/exness_historical_fill_telemetry_diagnostic.py
"""
...
# =============================================================================
# Diagnostic repository-root bootstrap
# =============================================================================

_bootstrap_mod: Any = importlib.import_module(
    (
        f"{__package__}._repository_bootstrap"
        if __package__
        else "_repository_bootstrap"
    )
)

ensure_repository_root_on_sys_path: Any = (
    _bootstrap_mod.ensure_repository_root_on_sys_path
)

PROJECT_ROOT = ensure_repository_root_on_sys_path(
    Path(
        __file__
    ).resolve().parent
)
