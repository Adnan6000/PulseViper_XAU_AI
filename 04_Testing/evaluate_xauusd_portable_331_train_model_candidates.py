...
class EvaluatorContractError(
    ValueError
):
    """
    Raised when the frozen evaluator
    contract is violated.
    """


class CandidateFoldFailure(
    RuntimeError
):
    """
    Raised when one candidate cannot
    be evaluated safely on one fold.
    """
    ...
    def validate(
        ...
        if any(
            ...
            raise EvaluatorContractError(
                "Fold boundaries and purge_rows "
                "must be integers."
            )

        if not self.fold_id:
            raise EvaluatorContractError(
                "fold_id must be non-empty."
            )

        if self.purge_rows < 0:
            raise EvaluatorContractError(
                "purge_rows must be >= 0."
            )
        ...
        if not (
            ...
            raise EvaluatorContractError(
                f"Invalid validation range for "
                f"{self.fold_id}."
            )
        ...
        if (
            ...
            raise EvaluatorContractError(
                f"Insufficient purge gap in "
                f"{self.fold_id}: "
                f"required={self.purge_rows}, "
                f"actual={actual_gap}."
            )
...
def load_frozen_candidate_registry(
    path: str | Path,
) -> dict[str, Any]:
    registry_path = Path(
        path
...
def validate_frozen_candidate_registry(
    ...
    if (
        ...
        raise EvaluatorContractError(
            "Results-driven candidate addition "
            "must be blocked."
        )
    ...
    if (
        ...
        raise EvaluatorContractError(
            "Results-driven hyperparameter changes "
            "must be blocked."
        )
    ...
    if (
        ...
        raise EvaluatorContractError(
            "All candidates must use the exact "
    ...
    for key in blocked_flags:
        if (
            ...
            raise EvaluatorContractError(
                "Anti-overfit flag must "
                f"be false: {key}"
            )

    candidates = contract.get(
        "candidates"
    )
    ...
    if (
        ...
        raise EvaluatorContractError(
            "Walk-forward evaluation must "
            "be chronological only."
        )
    ...
    if (
        ...
        raise EvaluatorContractError(
            "Synthetic evaluator tests must "
            "gate real candidate fit."
        )

    registry_sha = fingerprint.get(
        "sha256"
    )
    ...
    if (
        ...
        raise EvaluatorContractError(
            "Registry contract content does "
            "not match its canonical SHA256 "
            "fingerprint."
        )
...
def _as_tradeable_vector(
    ...
    if not np.isin(
        ...
        raise EvaluatorContractError(
            "target_tradeable contains values "
            "outside {0, 1}."
        )
...
def validate_train_only_inputs(
    ...
    if not np.array_equal(
        ...
        raise EvaluatorContractError(
            "target_tradeable linkage does "
            "not match target_class != 0."
        )
...
def validate_fold_specs(
    ...
    if (
        ...
        raise EvaluatorContractError(
            f"Expected {required_fold_count} "
            f"folds, received {len(folds)}."
        )
    ...
    for fold in folds:
        ...
        if (
            ...
            raise EvaluatorContractError(
                f"Fold {fold.fold_id} "
                "purge_rows must equal "
                f"frozen value "
                f"{required_purge_rows}."
            )
...
def _inverse_frequency_sample_weight(
    ...
    if (
        ...
        raise CandidateFoldFailure(
            "Cannot compute class-balance "
            "weights with fewer than "
            "two classes."
        )
...
def compute_fold_metrics(
    ...
    if (
        ...
        raise CandidateFoldFailure(
            "Directional recall output "
            "must contain exactly SHORT "
            "and LONG recalls."
        )
